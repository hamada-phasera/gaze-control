...
- `--max-speed 1500` … 点間の移動の速さ px/秒（固定は注視ロックが担うので速めでOK。小さいほど遅い）
- `--eye right` / `--eye-weight 0.8` … **利き目**を優先（両目平均だと利き目側にズレる）。⚠️ 変えたら **`--recalibrate` 推奨**
- `--offset-x 40` / `--offset-y -30` … 系統的なズレの微調整（右が正/下が正。左下にズレるなら x+ y−）
...
- `--down-smooth 0.6` … **下を見るほど縦を強く平滑化**（下端でカーソルが分散・ブレるのを抑える）。reachは保ったままノイズだけ抑えたい時に
