"""視線比率（画像平面・左右符号そろえ）の正規化テスト。

- 純粋なヨー/ピッチ単独の頭部回転では比率が（縮み相殺で）不変であること
- 左右の目（目頭→目尻が逆向き）で同じ視線が同符号になること（相殺しない＝縦リーチの主因）
- z=0（正面）では非反転側の目が素の2Dと一致すること

注: 横を3D（目頭→目尻軸）に投影すると実顔の目尻の奥行きで横信号が潰れるため、
横も縦も画像平面で測る。複合の面外回転では近似不変。
...
class TestRotationInvariance:
    ...
    def test_combined_yaw_pitch_invariant(self):
        ...
        # 画像平面計測なので、純粋なヨー/ピッチ単独では縮みが相殺して厳密不変だが、
        # 複合の面外回転では小さな誤差が残る（実顔の横信号を殺さないためのトレードオフ）。
        assert rotated[0] == pytest.approx(base[0], abs=0.02)
