...
# 既定0: --gaze-3d で目の縦信号が十分動くようになったので、頭ピッチでYを乗っ取ると
# 「Yがうなずきで動く＝不正確」になる（実測 corr(y,head_pitch)≈-0.76）。
# 首で届かせたい人だけ --head-assist 30 等で明示的に有効化する。
HEAD_PITCH_ASSIST = 0.0

# 視線比率の安全クランプ: 瞬きで目の高さが0に近づくと縦比率が発散する
# （実測 iris_y が -4〜+9 まで爆発）。片目の比率をこの絶対値にクランプして
# スパイクを抑える。通常の視線は ±0.5 程度なので 1.5 は十分な余裕。
GAZE_RATIO_CLAMP = 1.5
...
# 視線比率の改良経路（--gaze-3d）: 画像平面で測りつつ、左右の目で符号をそろえて
# 両目が補強し合うようにする（素の2Dは目頭→目尻の向きが左右逆＝縦横とも相殺しがち）。
# これが縦リーチ拡大の主因。z(奥行き)は使わない: 目尻がこめかみ側で奥にあるため
# 3D軸へ投影すると横信号が顔の奥行き構造に潰される（実測でiris_xが固定化）。
# True=有効。信号の符号/スケールが既定2Dと変わるので有効化時は要 --recalibrate。
GAZE_3D = False
